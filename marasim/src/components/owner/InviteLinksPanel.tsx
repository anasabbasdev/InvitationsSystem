  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-zinc-800 bg-zinc-900 p-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-zinc-200">روابط الدعوة الخاصة (Controlled Link)</h2>
        <button
          type="button"
      ...
      {!slug && (
        <p className="text-xs text-zinc-500">لا توجد دعوة مرتبطة بهذه المناسبة بعد.</p>
      )}
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-2">
            <input
              ...
              className="col-span-2 rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs text-white outline-none focus:border-amber-500"
            />
            <input
              name="guestName"
              placeholder="اسم الضيف (اختياري)"
              className="rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs text-white outline-none focus:border-amber-500"
            />
            <select
              name="side"
              defaultValue=""
      ...
      {links.length === 0 ? (
        <p className="text-xs text-zinc-500">لا توجد روابط خاصة بعد.</p>
      ) : (
        <ul className="flex flex-col gap-2">
          {links.map((link) => (
            <li
              key={link.id}
              className="flex flex-col gap-1.5 rounded-md border border-zinc-800 bg-zinc-950/50 p-3 text-xs"
            >
              ...
              <div className="flex items-center gap-2 pt-1">
                <button
                  ...
                  disabled={!slug}
                  className="rounded-md border border-zinc-700 px-2.5 py-1 text-[11px] text-zinc-300 hover:bg-zinc-800 disabled:opacity-40"
                >
                ...
                {link.status === "active" && (
                  <form action={disableInviteLinkAction.bind(null, eventId, link.id)}>
                    <button
                      type="submit"
                      className="rounded-md border border-red-900 px-2.5 py-1 text-[11px] text-red-300 hover:bg-red-950/50"
                    >
                      تعطيل
                    </button>
                  </form>
