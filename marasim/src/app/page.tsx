...
const DEMOS = [
  { slug: "ws-royal-demo", label: "زفاف ملكي — داكن", desc: "Royal Dark · Public RSVP" },
  { slug: "ws-floral-demo", label: "زفاف سينمائي — زهري", desc: "Cinematic Floral · Event B" },
      ...
      <section className="mx-auto flex max-w-3xl flex-col gap-10 px-6 py-16">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
          ...
          <a
            ...
            className="rounded-full border border-zinc-700 px-8 py-4 text-center text-base text-zinc-200 transition hover:bg-zinc-900"
          >
            طلب عرض سعر
          </a>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {[
            ...
            <div key={item.title} className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
              <h2 className="mb-1 text-sm font-semibold" style={{ color: "#C9A24D" }}>
                {item.title}
              </h2>
        ...
        <div>
          <h2 className="mb-3 text-sm font-semibold text-zinc-300">الديموهات</h2>
          <ul className="flex flex-col gap-2">
            {DEMOS.map((demo) => (
              <li key={demo.slug}>
                <Link
                  href={`/i/${demo.slug}`}
                  className="flex items-center justify-between rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-3 transition hover:border-amber-700/40"
                >
                  <div>
                    <p className="font-medium">{demo.label}</p>
                    <p className="text-xs text-zinc-500">{demo.desc}</p>
                  </div>
                  <span className="text-xs text-amber-500">فتح ←</span>
                </Link>
              </li>
            ))}
          </ul>
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-5 text-sm text-zinc-500">
          <h2 className="mb-2 font-semibold text-zinc-300">خدمات إضافية (مراسِم)</h2>
        ...
        <div className="flex flex-wrap justify-center gap-4 text-xs text-zinc-600">
          <Link href="/owner/login" className="hover:text-zinc-400">
            دخول المالك
          </Link>
          {process.env.NODE_ENV !== "production" && (
            <Link href="/lab/test-hub" className="hover:text-zinc-400">
              Test Hub (dev)
            </Link>
          )}
        </div>
